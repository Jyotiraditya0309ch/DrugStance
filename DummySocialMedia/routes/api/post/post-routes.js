  ...
  try {
    let { id } = req.session,
      ...
      insert = {
        user: id,
        description: desc,
        imgSrc: 'C:\\Desktop\\CodeUtsava\\React-Instagram-Clone-2.0\\dist\\posts\\' + filename,
        filter,
        location,
        type,
        group_id: group && group !== 'undefined' ? group : null,
        post_time: new Date().getTime(),
        ipaddress: req.ip,
      };
      
    console.log('Inserting Post:', insert);
    console.log(req.ip); // Debugging IP address

    // Update the user's IP address in the users table
    await db.query('UPDATE users SET ipaddress = ?, location = ? WHERE id = ?', [req.ip, location, id]);
